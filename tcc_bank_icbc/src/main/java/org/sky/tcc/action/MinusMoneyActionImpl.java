...
public class MinusMoneyActionImpl extends BaseService implements MinusMoneyAction {
	 ...
	 */
	@Autowired
	private TransferMoneyDAO fromAccountDAO;
	...
	@Override
	public boolean prepareMinus(BusinessActionContext businessActionContext, long accountNo, double amount) {
		// 分布式事务ID
		...
		return fromDsTransactionTemplate.execute(new TransactionCallback<Boolean>() {
			...
			public Boolean doInTransaction(TransactionStatus status) {
				try {
					// 校验账户余额
					AccountBean account = fromAccountDAO.getAccountForUpdate(accountNo);
					if (account == null) {
					...
					logger.error("error occured in MinusMoneyActionImpl.prepareMinus: " + t.getMessage(), t.getCause());
		...
		return fromDsTransactionTemplate.execute(new TransactionCallback<Boolean>() {
			...
			public Boolean doInTransaction(TransactionStatus status) {
				try {
					...
					logger.error("error occured in MinusMoneyActionImpl.commit: " + t.getMessage(), t.getCause());
					status.setRollbackOnly();
					status.setRollbackOnly();
					return false;
				}
		...
		return fromDsTransactionTemplate.execute(new TransactionCallback<Boolean>() {
			...
			public Boolean doInTransaction(TransactionStatus status) {
				try {
					...
					account.setFreezedAmount(account.getFreezedAmount() - amount);
					fromAccountDAO.updateFreezedAmount(account);
					logger.info(String.format("Undo prepareMinus account[%s] amount[%f], dtx transaction id: %s.",
							accountNo, amount, xid));
					return true;
				} catch (Throwable t) {
					logger.error("error occured in MinusMoneyActionImpl.rollback: " + t.getMessage(), t.getCause());
					status.setRollbackOnly();
					return false;
				}
			}
